...
public class MakeBranch : MonoBehaviour
{
            ...
            if (i < lineRenderermmm.positionCount)
            {
               // Debug.Log("Next Flower Index = " + nextFlowerIndex + " i = " + i + " Point position = " + m_allPoints[i]);
                ...
                if ((nextFlowerIndex <= i))
                {
                    ...
                    float sizeFleurs = Random.Range(10.0f, 18.0f);
                    ...
                    switch (type)
                    {
                        ...
                        case 5: // MARGUERITE
                            indexSprite = 2;
                            sizeFleurs = Random.Range(5.0f, 8.0f);
                ...
                if ((nextFeuilleIndex <= i))
                {
                    ...
                    float sizeFeuilles = Random.Range(2.0f, 20.0f);

                    switch (type)
                    {
                        case 1: // VIOLETTE
                            indexSprite = 1;
                            Debug.Log("In Feuille");
                            sizeFeuilles = Random.Range(3.0f, 10.0f);
                        ...
                        case 5: // MARGUERITE
                            indexSprite = 2;
                            sizeFeuilles = Random.Range(15.0f, 30.0f);
    ...
    public void createFlowers(Vector3 pos, Vector3 info) //in info x: angle, y: size, z: index
    {
        ...
        anim.runtimeAnimatorController = Resources.Load<RuntimeAnimatorController>("Textures/espece" + (int)info.z + "/fleur" + (int)info.z);
        int tex = (((int)info.z - 1) * 6) + Random.Range(1, 7);
        clone.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("Textures/espece" + (int)info.z + "/fleur" + tex);
    }
    ...
    public void createLeafs(Vector3 pos, Vector3 info) //in info x: angle, y: size, z: index
    {
        ...
        Animator anim = clone.GetComponent<Animator>();
        anim.runtimeAnimatorController = Resources.Load<RuntimeAnimatorController>("Textures/espece" + (int)info.z + "/feuille" + (int)info.z);
        int tex = (((int)info.z - 1) * 6) + Random.Range(1, 7);
        clone.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("Textures/espece" + (int)info.z + "/feuille" + tex);
    }
}
