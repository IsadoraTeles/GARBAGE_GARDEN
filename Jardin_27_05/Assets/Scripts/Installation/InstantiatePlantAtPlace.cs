...
public class InstantiatePlantAtPlace : MonoBehaviour
{
    ...
    public void createPlant(Vector3 m_origin, int m_type)
    {
        switch (m_type)
        {
            ...
            case 8: // BUISSON
                float angleB = 0.0f;
                float sizeB = Random.Range(25.0f, 40.0f);
                int indexB = 1;
            ...
            case 9: // BUISSON
                float angleB2 = 0.0f;
                float sizeB2 = Random.Range(10.0f, 20.0f);
                int indexB2 = 2;
    ...
    public void createBuisson(Vector3 pos, Vector3 info) //in info x: angle, y: size, z: index
    {
        ...
        anim.runtimeAnimatorController = Resources.Load<RuntimeAnimatorController>("Textures/buissons/buisson" + (int)info.z);
        obj.GetComponent<SpriteRenderer>().sprite = Resources.Load<Sprite>("Textures/buissons/buisson" + (int)info.z);
    }
    ...
    public void destroyPlante()
    {
        if (plants.Count != 0)
        {
            for (int i = 0; i < plants.Count; i++)
            {
                ...
                {
                    if (plants[i].GetComponent<MakePlants>().lifeTime < 4)
                    {
                ...
                {
                    if (plants[i].GetComponent<makeBrancheLsystem>().lifeTime < 4)
                    {
                        //Debug.Log("DESTROOOOOOOOOOOOOOOOOY");
                        ps = (ParticleSystem)Instantiate(ps);
        ...
        if (buissons.Count != 0)
        {
            ...
            {
                if (buissons[i].GetComponent<lifetimeBuisson>().lifetime < 4)
                {
                    ps = (ParticleSystem)Instantiate(ps);
